...
const EnvironmentalConditions = () => {
  ...
  const avgTemp = records.length
    ? (records.reduce((sum, r) => sum + parseFloat(r.temperature_celsius || 0), 0) / records.length).toFixed(2)
    : 0;

  const avgHumidity = records.length
    ? (records.reduce((sum, r) => sum + parseFloat(r.humidity_percent || 0), 0) / records.length).toFixed(2)
    : 0;
  ...
  return (
    <div className="container mt-5">
      <h3 className="text-center text-decoration-underline">Environmental Conditions Dashboard</h3>
      ...
      <div className="row mt-4">
        <div className="col-md-4">
          <div className="card shadow-sm p-3">
            ...
            <div className="text-center"><h3>{avgTemp} °C</h3></div>
          </div>
        </div>
        <div className="col-md-4">
          <div className="card shadow-sm p-3">
            <h5 className="text-center">Average Humidity</h5>
            <div className="text-center"><h3>{avgHumidity} %</h3></div>
          </div>
        </div>
        <div className="col-md-4">
          <div className="card shadow-sm p-3">
            <h5 className="text-center">Unique Batches</h5>
            <div className="text-center"><h3>{uniqueBatches}</h3></div>
          </div>
        </div>
      </div>
      ...
      <Link to="/addenvironment" className="btn btn-outline-dark my-4">Add Environmental Record</Link>
